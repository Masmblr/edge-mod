...
void AGeneric_CreepCheck( gentity_t *self )
{
	...
	spawn = self->parentNode;
  	...
  	if( self->s.modelindex != BA_H_REACTOR 
		...
		|| self->s.modelindex != BA_A_POD_STUMP
		|| self->s.modelindex != BA_A_POD
		|| self->s.modelindex != BA_A_NET_SPIKE
		|| self->s.modelindex != BA_A_NET
		|| self->s.modelindex != BA_A_TENDON_LARGE
		|| self->s.modelindex != BA_A_TENDON_MEDIUM 
		|| self->s.modelindex != BA_A_TENDON_SMALL 
		|| self->s.modelindex != BA_A_PANZER_LARGE
		|| self->s.modelindex != BA_A_PANZER_MEDIUM
		|| self->s.modelindex != BA_A_PANZER_SMALL) {
			return;
	}
...
{
	#define PUSH_DMGRADIUS 110
	#define PUSH_REPEAT 500
	#define PUSH_RANGE 140
	#define PUSH_FORCE 800
	...
	if( self->spawned && self->health > 0 && self->powered )
	{
		...
		for( i = 0; i < num; i++ )
		{
			...
			if( enemy->client->notrackEndTime >= level.time )
				continue;
			...
			// Event + Animation
			G_AddEvent( self, EV_FORCE_FIELD, DirToByte( dir ) );
...
void G_Suck( gentity_t *self )
{
  ...
  int i, num;
  gentity_t *enemy;
  vec3_t start, dir, end, pushVec;
  qboolean didSuck = qfalse;
  ...
  for( i = 0; i < num; i++ )
  {
    ...
    if( enemy == self || enemy == self->parent )
      continue;
    ...
    if( enemy->client->ps.stats[ STAT_TEAM ] != TEAM_HUMANS )
      continue;
    ...
    VectorNormalize( dir );
    VectorScale( dir, 200, pushVec );
    VectorAdd( enemy->client->ps.velocity, pushVec, enemy->client->ps.velocity );
    VectorCopy( dir, enemy->movedir );
  ...
  {
    // Event and Animation
    G_AddEvent( self, EV_ALIEN_SLIME, DirToByte( dir ) );
    ...
    for( i = 0; i < num; i++ )
    {
      gentity_t *opfer = &g_entities[ entityList[ i ] ];
      ...
      tent->s.generic1 = self->s.number;
      tent->s.clientNum = opfer->s.number;
...
void G_Scan( gentity_t *self )
{
  ...
  for( i = 0; i < num; i++ )
  {
    ...
    if( !enemy->client ||
        ...
        enemy->client->notrackEndTime >= level.time ||
        enemy->client->ps.stats[ STAT_TEAM ] != TEAM_HUMANS ||
        !G_Visible( self, enemy, CONTENTS_SOLID ) )
    {
      continue;
    }

    //G_AddEvent( self, EV_ABSCESS_EXPLODE, 0 );
    self->health = -999;
  ...
  if( ent->client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
  {
    ...
    if( BG_Buildable( buildable )->creepTest )
    {
		if( buildable != BA_H_REACTOR 
			&& buildable != BA_H_REPEATER 
			&& buildable != BA_A_POD_STUMP
			&& buildable != BA_A_POD
			&& buildable != BA_A_NET_SPIKE
			&& buildable != BA_A_NET
			&& buildable != BA_A_TENDON_LARGE
			&& buildable != BA_A_TENDON_MEDIUM 
			&& buildable != BA_A_TENDON_SMALL 
			&& buildable != BA_A_PANZER_LARGE
			&& buildable != BA_A_PANZER_MEDIUM
			&& buildable != BA_A_PANZER_SMALL) {
    ...
    const vec3_t origin, const vec3_t normal, const vec3_t angles )
{
  ...
  switch( buildable )
  {
	...
	case BA_H_LIGHT:
      built->think = G_Push;
	  built->nextthink = level.time + 100; // oder PUSH_REPEAT
	  built->die = HSpawn_Die;
      break;
